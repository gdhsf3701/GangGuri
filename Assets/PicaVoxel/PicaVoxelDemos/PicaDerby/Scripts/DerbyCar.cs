    ...
    void Update ()
	{
        ...
        if (!resetting)
	    {
	        if (IsPlayerControlled)
	        {
	            ...
	            mainCamera.transform.localPosition = Vector3.Slerp(mainCamera.transform.localPosition, cameraStartPosition + new Vector3(steerAmount*(rigidBody.linearVelocity.magnitude*0.1f), 0f, 0f), Time.deltaTime * 10f);
